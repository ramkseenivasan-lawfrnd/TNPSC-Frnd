...
const InstallPrompt: React.FC<InstallPromptProps> = ({ language }) => {
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-2rem)] max-w-md animate-in slide-in-from-bottom-10">
      <div className="bg-[#003366] text-white p-5 rounded-[2rem] shadow-2xl border border-white/10 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#FFD700] rounded-xl flex items-center justify-center text-[#003366] shrink-0">
            <Sparkles size={20} />
          </div>
          <div>
            ...
            <p className="text-[10px] text-blue-200 font-bold opacity-80">
              {language === 'TN' ? 'வேகமான அனுபவத்திற்கு இதை முகப்புத் திரையில் சேர்க்கவும்.' : 'Add to home screen for a faster, app-like experience.'}
            </p>
          </div>
        </div>
